...
void  MyTime::askHour()
{
	...
	cout << endl << "Is AM (press 1; otherwise PM): ";
	cin >> d;
	if (d != '1')
		this->setPM();
}
...
bool MyTime::operator > (const MyTime& aTime) const
{
	return (((this->isAM() && aTime.isAM()) ? this->hour > aTime.hour:-(this->hour) > -(aTime.hour)) ||
		(this->hour == aTime.hour && this->minutes > aTime.minutes) ||
		(this->hour == aTime.hour && this->minutes == aTime.minutes && this->seconds > aTime.seconds));
}
bool MyTime::operator < (const MyTime& aTime) const
{
