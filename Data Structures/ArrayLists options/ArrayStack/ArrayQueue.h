...
public:
	...
	void display() const;
	int getInteger(ArrayQueue aQueue);
}; // end ArrayQueue
...
void ArrayQueue<ItemType>::display() const
{
	...
	{
		cout << this->items[front];
	}

	else
	{
		for (int x = this->front; x <= this->count; x++)
		{
			//mi display
			if (x < MAX_QUEUE)
			{
			...
			else if (x == MAX_QUEUE && this->front != 0)
			{
				for (int y = 0; y <= this->back; y++)
				{
					cout << this->items[y];
					if (y != back)
						cout << ",";
			...
			//display de companero
			//cout << x % MAX_QUEUE << ": " << items[x % MAX_QUEUE] << endl;
		}
		cout << endl;
	}
}
...
template <class ItemType>
int ArrayQueue<ItemType>::getInteger(ArrayQueue<ItemType> aQueue)
{
	//get first digit ignoring any leading blank
	/*do {
		aQueue.dequeue(ch);
	} while (ch == NULL);*/
