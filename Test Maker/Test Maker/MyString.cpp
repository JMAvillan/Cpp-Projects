...
MyString MyString::trimAllSpaces() const
{
	...
	for (int i = 0; i < this->len(); i++)
		if ((this->s)[i] != '\t' && (this->s)[i] != ' ')
			r = r + (this->s)[i];		//every time there's a blank space it gets ignored
										//this takes the already existing string and takes the next
										//non blank space and puts it at the end of the string using
										//the '+' operator 
	return r;
}
